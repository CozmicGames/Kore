...
fun WriteStream.writeInt8(v: Int) = writeByte((v and 0xFF).toByte())

fun WriteStream.writeInt16LE(v: Int) {
    writeInt8(v and 0xFF)
    writeInt8((v ushr 8) and 0xFF)
}

fun WriteStream.writeInt24LE(v: Int) {
    writeInt8(v and 0xFF)
    writeInt8((v ushr 8) and 0xFF)
    writeInt8((v ushr 16) and 0xFF)
}

fun WriteStream.writeInt32LE(v: Int) {
    writeInt8(v and 0xFF)
    writeInt8((v ushr 8) and 0xFF)
    writeInt8((v ushr 16) and 0xFF)
...
fun WriteStream.writeInt64LE(v: Long) {
    writeInt32LE(v.toInt())
    writeInt32LE((v ushr 32).toInt())
}

fun WriteStream.writeFloat32LE(v: Float) = writeInt32LE(v.toRawBits())

fun WriteStream.writeFloat64LE(v: Double) = writeInt64LE(v.toRawBits())

fun WriteStream.writeInt16BE(v: Int) {
    writeInt8((v ushr 8) and 0xFF)
    writeInt8(v and 0xFF)
}

fun WriteStream.writeInt24BE(v: Int) {
    writeInt8((v ushr 16) and 0xFF)
    writeInt8((v ushr 8) and 0xFF)
    writeInt8(v and 0xFF)
}

fun WriteStream.writeInt32BE(v: Int) {
    writeInt8((v ushr 24) and 0xFF)
    writeInt8((v ushr 16) and 0xFF)
    writeInt8((v ushr 8) and 0xFF)
    writeInt8(v and 0xFF)
...
fun WriteStream.writeInt64BE(v: Long) {
    writeInt32BE((v ushr 32).toInt())
    writeInt32BE(v.toInt())
}

fun WriteStream.writeFloat32BE(v: Float) = writeInt32BE(v.toRawBits())

fun WriteStream.writeFloat64BE(v: Double) = writeInt64BE(v.toRawBits())

fun WriteStream.writeInt16(v: Int, endianness: Files.Endianness = Kore.files.nativeEndianness) = when (endianness) {
    Files.Endianness.LITTLE_ENDIAN -> writeInt16LE(v)
    Files.Endianness.BIG_ENDIAN -> writeInt16BE(v)
}

fun WriteStream.writeInt24(v: Int, endianness: Files.Endianness = Kore.files.nativeEndianness) = when (endianness) {
    Files.Endianness.LITTLE_ENDIAN -> writeInt24LE(v)
    Files.Endianness.BIG_ENDIAN -> writeInt24BE(v)
}

fun WriteStream.writeInt32(v: Int, endianness: Files.Endianness = Kore.files.nativeEndianness) = when (endianness) {
    Files.Endianness.LITTLE_ENDIAN -> writeInt32LE(v)
    Files.Endianness.BIG_ENDIAN -> writeInt32BE(v)
}

fun WriteStream.writeInt64(v: Long, endianness: Files.Endianness = Kore.files.nativeEndianness) = when (endianness) {
    Files.Endianness.LITTLE_ENDIAN -> writeInt64LE(v)
    Files.Endianness.BIG_ENDIAN -> writeInt64BE(v)
}

fun WriteStream.writeFloat32(v: Float, endianness: Files.Endianness = Kore.files.nativeEndianness) = when (endianness) {
    Files.Endianness.LITTLE_ENDIAN -> writeFloat32LE(v)
    Files.Endianness.BIG_ENDIAN -> writeFloat32BE(v)
}

fun WriteStream.writeFloat64(v: Double, endianness: Files.Endianness = Kore.files.nativeEndianness) = when (endianness) {
