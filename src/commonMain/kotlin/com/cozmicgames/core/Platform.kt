...
/**
 * Only run when the current platform type matches the given platform type.
 *
 * @param platform The platform type to check against.
 * @param block The block to run if the platform matches.
 *
 * @return The result of the block.
 */
inline fun <R> runForPlatform(platform: PlatformType, block: () -> R) = if (Kore.platformType == platform)
    block()
else
    null