...
import com.cozmicgames.core.utils.extensions.clamp
import com.cozmicgames.core.utils.maths.fract
import com.cozmicgames.core.utils.maths.mix
import com.cozmicgames.core.utils.use
import com.cozmicgames.core.utils.maths.forEachLinePoint
...
class Image(val width: Int, val height: Int, val pixels: PixelData = PixelData(width, height)) {
     ...
     * @param srcHeight The height of the source image to draw to. Defaults to the image's height.
     */
    fun setImage(image: Image, x: Int = 0, y: Int = 0, width: Int = image.width, height: Int = image.height, srcX: Int = 0, srcY: Int = 0, srcWidth: Int = image.width, srcHeight: Int = image.height) {
        val srcU0 = srcX / (image.width - 1.0f)
        val srcV0 = srcY / (image.height - 1.0f)
        val srcU1 = (srcX + srcWidth - 1.0f) / (image.width - 1.0f)
        val srcV1 = (srcY + srcHeight - 1.0f) / (image.height - 1.0f)
...
/**
 * Sets the image data to the texture at the specified [level] and [layer].
 *
 * @param image The image to set the texture data to.
 * @param level The mipmap level to set the image data to.
 * @param layer The layer of the texture to set the image data to.
 */
fun GPUTexture2D.setImage(image: Image, level: Int = 0, layer: Int = 0) {
    Memory.of(*image.pixels.toByteArray()).use {
        setImage(it, level = level, layer = layer)
    }
}
