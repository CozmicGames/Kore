...
import com.cozmicgames.core.utils.concurrency.concurrentHashMapOf
import kotlin.collections.iterator
import kotlin.reflect.KClass
import kotlin.reflect.KProperty

class Context : Disposable {
    abstract class BaseInjector<T : Any>(protected val context: Context, protected val type: KClass<T>) {
    ...
    class NullableInjector<T : Any>(context: Context, cls: KClass<T>) : BaseInjector<T>(context, cls) {
        operator fun getValue(thisRef: Any, property: KProperty<*>): T? {
            return cachedValue
        }
    }

    open class Injector<T : Any>(context: Context, cls: KClass<T>) : BaseInjector<T>(context, cls) {
        open operator fun getValue(thisRef: Any, property: KProperty<*>): T {
            return requireNotNull(cachedValue)
        }
    }

    class LazyInjector<T : Any>(context: Context, cls: KClass<T>, private val default: () -> T) : Injector<T>(context, cls) {
    ...
    private val values = concurrentHashMapOf<KClass<*>, Any>()
    private val injectors = concurrentHashMapOf<KClass<*>, ArrayList<BaseInjector<*>>>()
