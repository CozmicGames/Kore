...
import com.cozmicgames.core.utils.collections.Resettable
...
class Matrix4x4 : Resettable {
    ...
    fun setToLookAt(eyeX: Float, eyeY: Float, eyeZ: Float, centerX: Float, centerY: Float, centerZ: Float, upX: Float, upY: Float, upZ: Float): Matrix4x4 {
        ...
        dirY = centerY - eyeY
        dirZ = centerZ - eyeZ
        val invDirLength: Float = 1.0f / sqrt(dirX * dirX + dirY * dirY + dirZ * dirZ)
        dirX *= invDirLength
        dirY *= invDirLength
        dirZ *= invDirLength
        var leftX: Float
        ...
        val invLeftLength: Float = 1.0f / sqrt(leftX * leftX + leftY * leftY + leftZ * leftZ)
        leftX *= invLeftLength
        leftY *= invLeftLength
        leftZ *= invLeftLength
        // up = direction x left
