 ...
 * @see [FloatBuffer]
 */
sealed class Buffer<T : Any>(size: Int, val valueSize: Int, private val allocator: MemoryAllocator) : Iterable<T>, Disposable {
    /**
     ...
     */
    var memory = allocator.alloc(size * valueSize)
        private set
    ...
    fun resize(size: Int) {
        ...
        val memory = allocator.alloc(size * valueSize)
        this.memory.copyTo(memory, min(this.memory.size, memory.size))
        this.memory.dispose()
        this.memory = memory
        this.size = size
    }
 ...
 */
class ByteBuffer(size: Int, allocator: MemoryAllocator = DefaultMemoryAllocator) : Buffer<Byte>(size, Memory.Companion.SIZEOF_BYTE, allocator) {
    /**
     ...
     */
    constructor(size: Int, isStackAllocated: Boolean) : this(size, if (isStackAllocated) StackMemoryAllocator else DefaultMemoryAllocator)
 ...
 * @param allocator The allocator to use to allocate the memory. Defaults to [alloc].
 */
class ShortBuffer(size: Int, allocator: MemoryAllocator = DefaultMemoryAllocator) : Buffer<Short>(size, Memory.Companion.SIZEOF_SHORT, allocator) {
    /**
 ...
 * @param allocator The allocator to use to allocate the memory. Defaults to [alloc].
 */
class IntBuffer(size: Int, allocator: MemoryAllocator = DefaultMemoryAllocator) : Buffer<Int>(size, Memory.Companion.SIZEOF_INT, allocator) {
    /**
 ...
 * @param allocator The allocator to use to allocate the memory. Defaults to [alloc].
 */
class LongBuffer(size: Int, allocator: MemoryAllocator = DefaultMemoryAllocator) : Buffer<Long>(size, Memory.Companion.SIZEOF_LONG, allocator) {
    /**
 ...
 * @param allocator The allocator to use to allocate the memory. Defaults to [alloc].
 */
class FloatBuffer(size: Int, allocator: MemoryAllocator = DefaultMemoryAllocator) : Buffer<Float>(size, Memory.Companion.SIZEOF_FLOAT, allocator) {
    constructor(size: Int, isStackAllocated: Boolean) : this(size, if (isStackAllocated) StackMemoryAllocator else DefaultMemoryAllocator)
