...
class StructArray<T : Struct>(val size: Int, supplier: () -> T) : Iterable<T>, Disposable {
     ...
     * @param supplier The supplier that will be used to create the structs. Structs initialized by the supplier must be created without an allocator.
     */
    constructor(size: Int, allocator: MemoryAllocator, supplier: () -> T) : this(size, supplier) {
        internalMemorySupplier = { allocator.alloc(size) }
        internalMemoryDestructor = { allocator.free(it) }
    }
     ...
     * @param supplier The supplier that will be used to create the structs. Structs initialized by the supplier must be created without an allocator.
     */
    constructor(size: Int, isStackAllocated: Boolean, supplier: () -> T) : this(size, if (isStackAllocated) StackMemoryAllocator else DefaultMemoryAllocator, supplier)
    ...
    internal var internalMemoryDestructor: ((Memory) -> Unit)? = null
    ...
    override fun dispose() {
        if (ownsMemory)
            internalMemoryDestructor?.invoke(memory)
    }
}