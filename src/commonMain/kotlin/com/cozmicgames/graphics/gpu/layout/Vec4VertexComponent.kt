...
class Vec4VertexComponent(name: String, normalized: Boolean, type: VertexLayout.AttributeType) : VertexComponent.Normal<Vector4>(name, type, 4, normalized) {
    override fun writeToMemory(value: Vector4, memory: Memory, offset: Int) {
        if (!isNormalized) {
            ...
            memory.setFloat(offset + Memory.SIZEOF_FLOAT * 3, value.w)
        } else when (type) {
            VertexLayout.AttributeType.BYTE -> {
                ...
                memory.setByte(offset + Memory.SIZEOF_BYTE * 3, (value.w * 0xFF).toInt().toByte())
            }
            VertexLayout.AttributeType.SHORT -> {
                ...
                memory.setShort(offset + Memory.SIZEOF_SHORT * 3, (value.w * 0xFFFF).toInt().toShort())
            }
            VertexLayout.AttributeType.INT -> {
                ...
                memory.setInt(offset + Memory.SIZEOF_INT * 3, (value.w * 0xFFFFFFFF).toInt())
            }
            else -> Kore.log.error(Vec4VertexComponent::class, "Unsupported vertex component configuration (Type: $type, Normalized: $isNormalized)")
        }
    }

    override fun readFromMemory(memory: Memory, offset: Int): Vector4 {
        val result = Vector4()
        ...
        if (!isNormalized) {
            ...
            result.w = memory.getFloat(offset + Memory.SIZEOF_FLOAT * 3)
        } else when (type) {
            VertexLayout.AttributeType.BYTE -> {
                ...
                result.w = memory.getByte(offset + Memory.SIZEOF_BYTE * 3) / 0xFF.toFloat()
            }
            VertexLayout.AttributeType.SHORT -> {
                ...
                result.w = memory.getShort(offset + Memory.SIZEOF_SHORT * 3) / 0xFFFF.toFloat()
            }
            VertexLayout.AttributeType.INT -> {
                ...
                result.w = memory.getInt(offset + Memory.SIZEOF_INT * 3) / 0xFFFFFFFF.toFloat()
            }
    ...
    override fun createVertexComponentObject() = Vector4()
}